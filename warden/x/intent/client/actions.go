...
func addFlagsFromMsg(msg sdk.Msg, cmd *cobra.Command) {
	...
	for i := 0; i < v.NumField(); i++ {
		...
		if strings.EqualFold(fieldName, "authority") {
			intentAuthority := authtypes.NewModuleAddress(types.ModuleName).String()
			cmd.Flags().String(flagName, intentAuthority, "Authority address (defaults to x/intent's address)")
			continue
		}
