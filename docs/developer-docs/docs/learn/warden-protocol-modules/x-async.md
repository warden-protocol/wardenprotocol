...
Prophets run on validator nodes separately from the [`wardend` process](/learn/glossary#warden-protocol-node), without blocking the consensus. Running a Prophet is optional for a validator.
