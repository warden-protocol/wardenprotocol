...
This guide provides steps for upgrading to the [Warden v1.0.0](https://github.com/warden-protocol/wardenprotocol/releases/tag/v1.0.0) release.

The network upgrade will take place on **March 3, 2026**. The Warden team is going to take the snapshot, export the state, and revert it back if needed.
...
1. Download the [wardend binary v1.0.0](https://github.com/warden-protocol/wardenprotocol/releases/tag/v1.0.0) and put it to `$WARDEN_HOME/cosmovisor/upgrades/v1.0.0/bin`:

   ```shell
   mkdir -p  $WARDEN_HOME/cosmovisor/upgrades/v1.0.0/bin
   cp $(which wardend) $WARDEN_HOME/cosmovisor/upgrades/v1.0.0/bin
   ```
...
2. Start the node:
   ...
   :::tip
   We recommend skipping the invariant checks to reduce the upgrade time. Future releases of the Cosmos SDK will include additional improvements to the crisis module.
   :::
...
1. Download the [wardend binary v1.0.0](https://github.com/warden-protocol/wardenprotocol/releases/tag/v1.0.0) or build it from the source code. See the installation instructions in the [Join Mainnet guide](../join-mainnet#1-install-the-binary).

2. Run Warden v0.7.6 until the upgrade height. The node will panic:

   ```shell
   ERR UPGRADE "v1.0.0" NEEDED at height: 5418000: upgrade to v1.0.0 and applying upgrade "v1.0.0" at height: 5418000
   ```

3. Stop the node and switch the binary to **Warden v1.0.0**. Restart the node:
...
1. Recover your node from a [backup](#1-verify--back-up).
   ...
   When applying a snapshot from the backup, keep your `.wardend/data/priv_validator_state.json` file. This will help you avoid being slashed due to double signing.

2. Restart the upgrade process.

:::note
If you proceed with the upgrade without recovering your node, the upgrade will likely complete successfully, but you'll get the `AppHash error` once the network is up.
:::
