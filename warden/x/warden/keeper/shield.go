...
func (w WardenShieldExpander) Expand(goCtx context.Context, ident *ast.Identifier) (*ast.Expression, error) {
	if strings.HasPrefix(ident.Value, "analyzer.") {
		return w.expandAnalyzers(goCtx, ident)
	}

	if ident.Value == "space.owners" {
		return w.expandSpaceOwners(goCtx)
...
func (w WardenShieldExpander) expandSpaceOwners(goCtx context.Context) (*ast.Expression, error) {
