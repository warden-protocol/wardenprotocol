...
func (k Keeper) EndBlocker(ctx context.Context) error {
	futures, err := k.futures.PendingFutures(ctx)
	if err != nil {
		return err
	}

	for _, f := range futures {
		k.p.AddFuture(prophettypes.Future{
			ID:      f.Id,
			Handler: f.Handler,
			Input:   f.Input,
		})
...
func (k Keeper) PrepareProposalHandler() sdk.PrepareProposalHandler {
	return func(ctx sdk.Context, req *cometabci.RequestPrepareProposal) (*cometabci.ResponsePrepareProposal, error) {
		...
		asyncTx, err := k.buildAsyncTx(ctx, req.LocalLastCommit.Votes)
		if err != nil {
...
func (k Keeper) PreBlocker() sdk.PreBlocker {
	return func(ctx sdk.Context, req *cometabci.RequestFinalizeBlock) (*sdk.ResponsePreBlock, error) {
		...
		for _, r := range tx.Results {
			if err := k.AddFutureResult(ctx, r.Id, proposer, r.Output); err != nil {
				return resp, nil
			}
		}
		log.Info("included future results", "count", len(tx.Results))
		...
		for _, v := range tx.ExtendedVotesInfo {
			...
			for _, vote := range asyncve.Votes {
				...
				if err := k.SetFutureVote(ctx, vote.FutureId, v.Validator.Address, vote.Vote); err != nil {
...
func (k Keeper) buildAsyncTx(votes []cometabci.ExtendedVoteInfo) ([]byte, error) {
	...
	results := make([]*types.FutureResult, len(pResults))
	for i, r := range pResults {
		results[i] = &types.FutureResult{
			Id:     r.ID,
			Output: r.Output,
		}
	}
