...
func NewRootCmd() *cobra.Command {
	...
	rootCmd := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, _ []string) error {
			...
			customAppTemplate, customAppConfig := wardendconfig.InitAppConfig(wardendconfig.BaseDenom, wardendconfig.EVMChainID)
...
func appExport(
	...
	if height != -1 {
		exampleApp = app.NewApp(logger, db, traceStore, false, appOpts, wardendconfig.EVMChainID, baseapp.SetChainID(chainID))

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = app.NewApp(logger, db, traceStore, true, appOpts, wardendconfig.EVMChainID, baseapp.SetChainID(chainID))
	}
