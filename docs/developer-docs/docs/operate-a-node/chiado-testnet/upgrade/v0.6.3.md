...
1. Download the `wardend` v0.6.3 binary and put it to `$WARDEN_HOME/cosmovisor/upgrades/v063/bin`:

   ```shell
   mkdir -p  $WARDEN_HOME/cosmovisor/upgrades/v060-to-v063/bin
   cp $(which wardend) $WARDEN_HOME/cosmovisor/upgrades/v063/bin
   ```
...
2. Run Warden v0.6.2 until the upgrade height. The node will panic:
   ...
   ```shell
   ERR UPGRADE "v0.6.3" NEEDED at height: 2041100: upgrade to v0.6.3 and applying upgrade "v0.6.3" at height: 3111000
   ```
