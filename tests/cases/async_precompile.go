...
func (c *Test_AsyncPrecompile) Run(t *testing.T, ctx context.Context, _ framework.BuildResult) {
	...
	addTaskTx, err := iAsyncClient.AddTask(alice.TransactOps(t, ctx, evmClient), "echo", []byte("USDT"), common.HexToAddress("0x0000000000000000000000000000000000000000"))
	require.NoError(t, err)

	addTaskReceipt, err := bind.WaitMined(ctx, evmClient, addTaskTx)
	require.NoError(t, err)
