...
function encodeSecp256k1Pubkey(pubkey: Uint8Array, urlType?: string): Secp256k1Pubkey {
...
function signerData(pubkey: any, sequence: number): ReadonlyArray<{readonly pubkey: any, readonly sequence: number}> {
    return [ pubkey, sequence ]
}

function coinData(denom: string, amount: string): readonly Coin[] {
    return [{ denom, amount }]
}

function txData(tx: Uint8Array): {readonly tx: Uint8Array} {
    return { tx }
}
...
class SigningCosmWasmClient extends CosmWasmClient {
...
(async function main() {
    ...
    const wallet = new DirectSecp256k1HdWallet(englishMnemonic, walletOpts)
    ...
    const wasm = fs.readFileSync("merkle-verifier/target/wasm32-unknown-unknown/release/fusion_merkle_verifier.wasm")
    ...
    const msg = {verify:{
        proof: fs.readFileSync("merkle.proof", "binary"),
        root: [31, 115, 121, 83, 151, 7, 188, 174, 160, 5, 100, 22, 141, 29, 77, 98, 107, 9, 183, 63, 138, 42, 54, 82, 52, 198, 45, 118, 63, 133, 77, 162],
        leaf_indices: [3, 4],
        leaf_hashes: [[24, 172, 62, 115, 67, 240, 22, 137, 12, 81, 14, 147, 249, 53, 38, 17, 105, 217, 227, 245, 101, 67, 100, 41, 131, 15, 175, 9, 52, 244, 248, 228], [63, 121, 187, 123, 67, 91, 5, 50, 22, 81, 218, 239, 211, 116, 205, 198, 129, 220, 6, 250, 166, 94, 55, 78, 56, 51, 123, 136, 202, 4, 109, 234]],
        leaf_count: 6
    }}
