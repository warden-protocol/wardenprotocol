...
This guide provides steps for upgrading to the [Warden v1.0.1](https://github.com/warden-protocol/wardenprotocol/releases/tag/v1.0.1) release.
...
1. Download the [wardend binary v1.0.1](https://github.com/warden-protocol/wardenprotocol/releases/tag/v1.0.1) and put it to `$WARDEN_HOME/cosmovisor/upgrades/v1.0.1/bin`:

   ```shell
   mkdir -p  $WARDEN_HOME/cosmovisor/upgrades/v1.0.1/bin
   cp $(which wardend) $WARDEN_HOME/cosmovisor/upgrades/v1.0.1/bin
   ```
...
1. Download the [wardend binary v1.0.1](https://github.com/warden-protocol/wardenprotocol/releases/tag/v1.0.1) or build it from the source code. See the installation instructions in the [Join Mainnet guide](../join-mainnet#1-install-the-binary).

2. Run Warden v0.7.6 until the upgrade height. The node will panic:

   ```shell
   ERR UPGRADE "v1.0.1" NEEDED at height: 5418000: upgrade to v1.0.1 and applying upgrade "v1.0.1" at height: 5418000
   ```

3. Stop the node and switch the binary to **Warden v1.0.1**. Restart the node:
