...
**Prerequisite**: in the repo's root, run `make install` to install the `fusiond` and `fusiond` binaries. When done, come back to this directory.
...
To run the tests, you can use the `test-helper.js` utility to test all suites under `ganache` or `ethermint` network. The `test-helper.js` will help you spawn an `fusiond` process before running the tests.
...
If you need to take more control, you can also run `fusiond` using:
...
Keep the terminal window open, go into any of the tests and run `yarn test-ethermint`. You should see `fusiond` accepting transactions and producing blocks. You should be able to query for any transaction via:

- `fusiond query tx <cosmos-sdk tx>`
...
- `qredo10jmp6sgh4cc6zt3e8gw05wavvejgr5pwtu750w` (Validator)
    - `0x7cB61D4117AE31a12E393a1Cfa3BaC666481D02E`
- `qredo1cml96vmptgw99syqrrz8az79xer2pcgp767p9e` (User 1)
    - `0xC6Fe5D33615a1C52c08018c47E8Bc53646A0E101`
- `qredo1jcltmuhplrdcwp7stlr4hlhlhgd4htqhgjpff2` (user 2)
    - `0x963EBDf2e1f8DB8707D05FC75bfeFFBa1B5BaC17`

Each with roughly 100 ETH available (1e18 photon).

Running `fusiond keys list --keyring-backend=test` should output:
...
[
  {
    "name": "localkey",
    "type": "local",
    "address": "qredo18de995q8qk0leqk3d5pzmg7tlxvj6tmsku084d",
  ...
  },
  {
    "name": "user1",
    "type": "local",
    "address": "qredo1mhtyk3cj7ly0rt8rc9zuj5pnnmw67gsapygwyq",
  ...
  },
  {
    "name": "user2",
    "type": "local",
    "address": "qredo1pa20g7lehr330vs5ent20slr3wyne4lsy8qae3",
