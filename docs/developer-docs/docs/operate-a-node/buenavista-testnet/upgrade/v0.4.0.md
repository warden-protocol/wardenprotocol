...
1. Recover your node from a [backup](#1-verify--back-up).  
    ...
    When applying a snapshot from the backup, keep your `./data/priv_validator_state.json` file. This will help you to avoid being slashed due to double signing. Note that you should also back up your private node keys.
