...
1. Download the `wardend` v0.6.1 binary and put it to `$WARDEN_HOME/cosmovisor/upgrades/v054-to-v061/bin`:

   ```shell
   mkdir -p  $WARDEN_HOME/cosmovisor/upgrades/v054-to-v061/bin
   cp $(which wardend) $WARDEN_HOME/cosmovisor/upgrades/v054-to-v061/bin
   ```
...
2. Run Warden v0.5.4 until the upgrade height. The node will panic:
   ...
   ```shell
   ERR UPGRADE "v0.6.1" NEEDED at height: 10754550: upgrade to v0.6.1 and applying upgrade "v0.6.1" at height: 10754550 
   ```
