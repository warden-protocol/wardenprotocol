...
This guide provides steps for upgrading to the [Warden v0.6.0](https://github.com/warden-protocol/wardenprotocol/releases/tag/v0.6.0) release, which enables the [`x/async` Warden module](/learn/warden-protocol-modules/x-async) on Chiado.
...
1. Download the `wardend` v0.6.0 binary and put it to `$WARDEN_HOME/cosmovisor/upgrades/v054-to-v060/bin`:
...
2. Run Warden v0.5.4 until the upgrade height. The node will panic:
   ...
   ```shell
   ERR UPGRADE "v0.6.0" NEEDED at height: 10754550: upgrade to v0.6.0 and applying upgrade "v0.6.0" at height: 10754550 
   ```
