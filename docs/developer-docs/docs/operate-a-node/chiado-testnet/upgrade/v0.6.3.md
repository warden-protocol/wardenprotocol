...
This guide provides steps for upgrading to the [Warden v0.6.3](https://github.com/warden-protocol/wardenprotocol/releases/tag/v0.6.3) release, which enables the [`x/async` Warden module](/learn/warden-protocol-modules/x-async) on Chiado.

The network upgrade will take place on **May 21, 2025**. The Warden team is going to take the snapshot, export the state, and revert it back if needed.
...
1. Download the `wardend` v0.6.3 binary and put it to `$WARDEN_HOME/cosmovisor/upgrades/v062-to-v063/bin`:

   ```shell
   mkdir -p  $WARDEN_HOME/cosmovisor/upgrades/v062-to-v063/bin
   cp $(which wardend) $WARDEN_HOME/cosmovisor/upgrades/v062-to-v063/bin
   ```
...
1. Download the [v0.6.3 wardend binary](https://github.com/warden-protocol/wardenprotocol/releases/tag/v0.6.3) or build it from the source code. See the installation instructions in the [Join Chiado guide](/operate-a-node/chiado-testnet/join-chiado#1-install).

2. Run Warden v0.6.2 until the upgrade height. The node will panic:

   ```shell
   ERR UPGRADE "v0.6.3" NEEDED at height: 3111000: upgrade to v0.6.3 and applying upgrade "v0.6.3" at height: 3111000
   ```

3. Stop the node and switch the binary to **Warden v0.6.3**. Restart the node:
