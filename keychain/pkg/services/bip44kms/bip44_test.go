...
func Test_NewKMS(t *testing.T) {
	...
	if _, err := NewBip44Keychain(mn, "password"); err != nil {
		t.Fatal(err)
	}
}

func Test_PublicKey(t *testing.T) {
	b, err := NewBip44Keychain(testMnemonic, "password")
	if err != nil {
		t.Fatal(err)
	}
	systems := []CryptoSystem{ECDSA, EDDSA}
...
func Test_MultiplePubKeys(t *testing.T) {
	...
	b, err := NewBip44Keychain(mn, "password")
	if err != nil {
		t.Fatal(err)
	}
	systems := []CryptoSystem{ECDSA, EDDSA}
		...
		for _, tt := range bip44tests {
			...
			t.Run(tt.name, func(t *testing.T) {
				...
				if err != nil {
					...
					// Validate key generation
					sig, _, err := b.Signature(&SigRequestData{KeyID: keyID, ID: iD, SigHash: msg}, sys)
					if err != nil {
						t.Fatal(err)
					}
