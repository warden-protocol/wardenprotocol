...
1. Download the [wardend binary v0.7.6](https://github.com/warden-protocol/wardenprotocol/releases/tag/v0.7.6) and put it to `$WARDEN_HOME/cosmovisor/upgrades/v0.7.6/bin`:

   ```shell
   mkdir -p  $WARDEN_HOME/cosmovisor/upgrades/v0.7.6/bin
   cp $(which wardend) $WARDEN_HOME/cosmovisor/upgrades/v0.7.6/bin
   ```
...
2. Run Warden v0.7.3 until the upgrade height. The node will panic:
   ...
   ```shell
   ERR UPGRADE "v0.7.6" NEEDED at height: 3265800: upgrade to v0.7.6 and applying upgrade "v0.7.6" at height: 3265800
   ```
