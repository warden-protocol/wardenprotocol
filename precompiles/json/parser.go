...
func (p *parser) parseContainer(ins []instruction, c *gabs.Container) ([]byte, error) {
	...
	for _, in := range ins {
		...
		if in.abiType != "" {
			...
			switch d := data.(type) {
			case (float64):
				data = makeGoNumber(in.abiType, d)
			case ([]any):
				data = makeGoArray(in.abiType, d)
			}
	...
	for _, in := range ins {
		if in.abiType == "fp" {
			...
			abiType, err := abi.NewType(typ, "", nil) // TODO: can we do better?
			if err != nil {
				return nil, fmt.Errorf("invalid abi type: %w", err)
			}
			abiArgs = append(abiArgs, abi.Argument{Type: abiType})
...
func makeGoNumber(abiType string, n float64) any {
	switch abiType {
	case "int256":
	...
	case "fp":
		return NewFixedPoint(n)
	}

	panic(fmt.Errorf("unsupport number abiType: %s", abiType))
}

func makeGoArray(abiType string, s []any) any {
	switch abiType {
	case "int256[]":
		fallthrough
	case "uint256[]":
		var res []*big.Int
		for _, arg := range s {
			i, _ := new(big.Float).SetFloat64(arg.(float64)).Int(nil)
			res = append(res, i)
		}
		return res
	case "bool[]":
	...
	panic(fmt.Errorf("unsupport abiType: %s", abiType))
}
