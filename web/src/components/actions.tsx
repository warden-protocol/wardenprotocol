  ...
  return (
    <div className="p-4 space-y-3">
      <span className="text-gray-800 italic">
        Action: intent started from an user that requires approvals from other
        users
      </span>
...
function Action(props: { action: Action }) {
  const color = props.action.completed
    ...
    <div className={`border-2 flex flex-col p-4 rounded ${color}`}>
      <span className="font-bold">
        ...
        <span className="text-xs font-medium">
          {props.action.completed ? "completed" : "in progress"}
        </span>
      </span>
    ...
    case "/fusionchain.identity.MsgAddWorkspaceOwner":
      return (
        <div className="flex flex-col">
          <span>
            Creator: <Address address={props.msg.creator} />
          </span>
          <span>
            Intent: add new owner (<Address address={props.msg.new_owner} />)
          </span>
        </div>
      );
    case "/fusionchain.identity.MsgRemoveWorkspaceOwner":
      return (
        <div className="flex flex-col">
          ...
          <span>
            Intent: remove owner (<Address address={props.msg.owner} />)
          </span>
        </div>
      );
    default:
      return <pre>{JSON.stringify(props.msg, null, 2)}</pre>;
    // case "/fusionchain.identity.MsgRemoveWorkspaceOwner":
    //   return (
    //   )
  }
