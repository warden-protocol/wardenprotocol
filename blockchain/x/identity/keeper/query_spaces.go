...
func (k Keeper) Spaces(goCtx context.Context, req *types.QuerySpacesRequest) (*types.QuerySpacesResponse, error) {
	...
	spaces := make([]types.Space, 0, query.DefaultLimit)
	store := ctx.KVStore(k.storeKey)
	spaceStore := prefix.NewStore(store, types.KeyPrefix(types.SpaceKey))

	pageRes, err := query.Paginate(spaceStore, req.Pagination, func(key []byte, value []byte) error {
		var space types.Space
		if err := k.cdc.Unmarshal(value, &space); err != nil {
			return err
		}
