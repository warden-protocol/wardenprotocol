...
func (k Keeper) processVE(ctx sdk.Context, fromAddr sdk.ConsAddress, ve types.AsyncVoteExtension) error {
	...
	if ve.UpdatePlugins {
		...
		removed, added, equals := diffSortedSlice(oldPlugins, ve.Plugins)
		...
		for _, r := range equals {
			if err := k.updateQueueWeight(ctx, QueueID(r), fromAddr, weight); err != nil {
				return fmt.Errorf("update %s weight for %s queue: %w", fromAddr.String(), r, err)
			}
		}
	} else {
		for _, r := range ve.Plugins {
			if err := k.updateQueueWeight(ctx, QueueID(r), fromAddr, weight); err != nil {
				return fmt.Errorf("no plugins registered/removed, updating %s weight for %s queue: %w", fromAddr.String(), r, err)
			}
		}
	}
