...
This guide provides steps for upgrading to the [Warden v0.6.0](https://github.com/warden-protocol/wardenprotocol/releases/tag/v0.6.0) release, which enable the [`x/async` Warden module](/learn/warden-protocol-modules/x-async) on Chiado.

The network upgrade will take place on **March 3, 2025**. The Warden team is going to take the snapshot, export the state, and revert it back if needed.
...
1. Download the `wardend` v0.6.0 binary and put it to `$WARDEN_HOME/cosmovisor/upgrades/v05-to-v06/bin`:

   ```shell
   mkdir -p  $WARDEN_HOME/cosmovisor/upgrades/v05-to-v06/bin
   cp $(which wardend) $WARDEN_HOME/cosmovisor/upgrades/v05-to-v06/bin
   ```
...
1. Download the [v0.6.0 wardend binary](https://github.com/warden-protocol/wardenprotocol/releases/tag/v0.6.0) or build it from the source code. See the installation instructions in the [Join Chiado guide](/operate-a-node/chiado-testnet/join-chiado#1-install).

2. Run Warden v0.5.4 until the upgrade height. The node will panic:

   ```shell
   ERR UPGRADE "v0.6.0" NEEDED at height: XXX: upgrade to v0.6.0 and applying upgrade "v0.6.0" at height: XXX 
   ```

3. Stop the node and switch the binary to **Warden v0.6.0**. Restart the node:
