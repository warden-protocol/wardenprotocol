...
This tutorial explains how to run the Warden binary, `wardend`, and join **Mainnet**.
...
1. Download the binary for your platform from the [release page](https://github.com/warden-protocol/wardenprotocol/releases/tag/v0.7.6) and unzip it. The archive contains the `wardend` binary.
...
- `config.toml`   
  ...
  ```
- `genesis.json`  
  Replace its contents with the correct [genesis settings](https://raw.githubusercontent.com/warden-protocol/networks/main/mainnet/genesis.json).
...
rm genesis.json
wget https://raw.githubusercontent.com/warden-protocol/networks/main/mainnet/genesis.json
```
...
1. From this RPC endpoint, you can get the trusted block height and hash:
   ...
   export LATEST_HEIGHT=$(curl -s "https://rpc.wardenprotocol.org/block" | jq -r .result.block.header.height)
   export BLOCK_HEIGHT=$((LATEST_HEIGHT - 2000))
   export TRUST_HASH=$(curl -s "https://rpc.wardenprotocol.org/block?height=$BLOCK_HEIGHT" | jq -r .result.block_id.hash)
   ```
...
After joining Mainnet, you can take these steps:
