...
2. Navigate to the directory containing the binary and initialize the chain:
...
2. Use our `just` script to build the `wardend` binary and install it to the `$GOPATH/bin` directory. Then initialize the chain.
