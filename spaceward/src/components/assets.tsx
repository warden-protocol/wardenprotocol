					...
					<div>
						<div className="space-y-3">
							{key.addresses?.map((addr) => {
								return (
									<Address address={addr.address} type={addr.type} keyId={key.key.id} />
								);
							})}
						</div>
					</div>
...
function Address({ address, type, keyId }: { address: string, type: AddressType, keyId: Long }) {
