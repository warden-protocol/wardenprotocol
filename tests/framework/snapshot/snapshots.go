...
func Build(t *testing.T, wardendPath, savePath string, opts BuildOptions) {
	...
	b := NewBuilder(t, w, "testing-node", opts.ChainID, "award", opts.KeyringBackend)
	...
	b.Replace("config/app.toml", "[evm]", "[evm]\n\nevm-chain-id = \""+opts.EVMChainID+"\"")
...
func (b *Builder) AddGenesisAccount(name, denom string) {
...
func (b *Builder) GenTx(name, denom string) {
	b.w.exec(b.t, "genesis",
		"gentx", name, denom,
		"--chain-id", b.chainID,
		"--keyring-backend", b.keyringBackend,
