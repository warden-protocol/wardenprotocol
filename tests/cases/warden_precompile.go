...
func (c *Test_WardenPrecompile) Run(t *testing.T, ctx context.Context, build framework.BuildResult) {
	...
	t.Run("work with keychains", func(t *testing.T) {
		...
		_, err := iWardenClient.NewKeychain(alice.TransactOps(t, ctx, evmClient), keychainName, keychainFees, keychainDescription, keychainUrl, keychainKeybaseId)
		require.NoError(t, err)
		...
		// add keychain writer
		_, err = iWardenClient.AddKeychainWriter(alice.TransactOps(t, ctx, evmClient), 1, alice.EthAddress(t))
		require.NoError(t, err)
		time.Sleep(4 * time.Second)
		keychain, err = iWardenClient.KeychainById(alice.CallOps(t), 1)
		if err != nil {
		...
		// add keychain admin
		_, err = iWardenClient.AddKeychainAdmin(alice.TransactOps(t, ctx, evmClient), 1, bob.EthAddress(t))
		require.NoError(t, err)
		time.Sleep(4 * time.Second)
		keychain, err = iWardenClient.KeychainById(alice.CallOps(t), 1)
		if err != nil {
		...
		_, err = iWardenClient.RemoveKeychainAdmin(alice.TransactOps(t, ctx, evmClient), 1, alice.EthAddress(t))
		require.NoError(t, err)
		time.Sleep(4 * time.Second)
		keychain, err = iWardenClient.KeychainById(alice.CallOps(t), 1)
		if err != nil {
		...
		_, err = iWardenClient.UpdateKeychain(bob.TransactOps(t, ctx, evmClient), 1, keychainName, keychainFees, keychainDescription, keychainUrl, keychainKeybaseId)
		require.NoError(t, err)
		time.Sleep(4 * time.Second)
		keychain, err = iWardenClient.KeychainById(alice.CallOps(t), 1)
		if err != nil {
	...
	t.Run("work with space", func(t *testing.T) {
		...
		_, err := iWardenClient.NewSpace(alice.TransactOps(t, ctx, evmClient), 0, 0, 0, 0, additionalOwners)
		require.NoError(t, err)
		time.Sleep(4 * time.Second)
	...
	t.Run("work with keys", func(t *testing.T) {
		...
		_, err = iWardenClient.FulfilKeyRequest(alice.TransactOps(t, ctx, evmClient), 1, pubKey)
		require.NoError(t, err)
		time.Sleep(4 * time.Second)
		...
		newKeyReqTx = alice.Tx(t, "warden new-action new-key-request --space-id 1 --keychain-id 1 --key-type 1 --max-keychain-fees \"1award\" --nonce 0")
		checks.SuccessTx(t, newKeyReqTx)
		_, err = iWardenClient.RejectKeyRequest(alice.TransactOps(t, ctx, evmClient), 2, "test reject reason")
		require.NoError(t, err)
		time.Sleep(4 * time.Second)
		...
		newSignReqTx = alice.Tx(t, "warden new-action new-sign-request --key-id 1 --input 'HoZ4Z+ZU7Zd08kUR5NcbtFZrmGKF18mSBJ29dg0qI44=' --max-keychain-fees \"1award\" --nonce 0")
		checks.SuccessTx(t, newSignReqTx)
		_, err = iWardenClient.RejectSignRequest(alice.TransactOps(t, ctx, evmClient), 2, "test reject reason")
		require.NoError(t, err)
		time.Sleep(4 * time.Second)
