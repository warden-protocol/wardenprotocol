...
func appExport(
	...
	if height != -1 {
		exampleApp = app.NewApp(logger, db, traceStore, false, appOpts, evmChainID, baseapp.SetChainID(chainID))

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = app.NewApp(logger, db, traceStore, true, appOpts, evmChainID, baseapp.SetChainID(chainID))
	}
