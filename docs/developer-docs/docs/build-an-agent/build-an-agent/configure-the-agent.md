---
sidebar_position: 3
---

# Configure the Agent

## Overview

After [creating a Warden Agent](create-a-new-agent), you can update the initial configuration at any moment, including the [A2A Agent Card](../warden-code/a2a-endpoints-and-methods#get-agent-card) metadata and [x402 payments](../warden-code/x402-payments).

This guide explains how to do it.

## Update Agent settings

To update your Agent's settings, take these steps:

1. Navigate to your project's root directory and initiate Warden Code:

   ```bash
   warden
   ```

2. Enter the configuration mode:
   
   ```bash
   /config
   ```

3. Select one of the available groups of settings and make the necessary updates:

   - **Identity**: Name, description, URL, version  

     :::note
     These settings affect the [Agent Card](../warden-code/a2a-endpoints-and-methods#get-agent-card) and the [ERC-8004 metadata](../register-on-erc-8004):
     - `src/public/.well-known/agent-card.json`
     - `src/public/.well-known/agent-registration.json`
     :::

   - **Server**: Port, host, Agent URL, API key, model

   - **Skills**: Agent capabilities advertised in the Agent Card

   - **Payments**: [x402 parameters](../warden-code/x402-payments#parameters) (wallets, pricing, networks)
 
   
![Configure your Agent using Warden Code](../../../static/img/configure-the-agent.png)

## Update the A2A Agent Card

An [A2A Agent Card](../warden-code/a2a-endpoints-and-methods#get-agent-card) is a JSON file that describes your Agent's capabilities. It enables clients and other Agents to discover and understand what the Agent can do.

Warden Code generates all Agents with Agent Cards, based on the information provided on Agent creation. If you need to change the Agent Card data later, use one of these methods:

- **`/config`**  
  Run `warden` and type the `/config` command. If you edit **Identity** and **Skills**, these updates will be reflected in the card.

- **`/build`**  
  Run `warden` and type the `/build` command to enter the AI-powered [build mode](implement-custom-logic#build-with-ai). Here you can ask the AI assistant to update the Agent Card for you.

- **Manual update**  
  You can find the Agent Card in `src/public/.well-known/agent-card.json`.

## Next steps

Now you can do the following:

- [Implement custom logic](implement-custom-logic)
- [Test the Agent locally](test-the-agent-locally)
